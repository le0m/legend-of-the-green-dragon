...
CREATE TABLE `accounts` (
  ...
  `laston` datetime NOT NULL DEFAULT (now()),
  ...
  `specialinc` varchar(50) NULL,
  `specialmisc` text NULL,
  `login` varchar(50) NOT NULL,
  `lastmotd` datetime NOT NULL DEFAULT (now()),
  `playerfights` int(11) unsigned NOT NULL DEFAULT '3',
  `lasthit` datetime NOT NULL DEFAULT (now()),
  ...
  `emailvalidation` varchar(32) NULL,
  `sentnotice` int(11) NOT NULL DEFAULT '0',
  `prefs` text NULL,
  `pvpflag` datetime NOT NULL DEFAULT (now()),
  ...
  `biotime` datetime NOT NULL DEFAULT (now()),
  ...
  `clanjoindate` datetime NOT NULL DEFAULT (now()),
  PRIMARY KEY (`acctid`),
  KEY `name` (`name`),
  KEY `level` (`level`),
  KEY `login` (`login`),
...
CREATE TABLE `logdnet` (
  ...
  `lastping` datetime NOT NULL DEFAULT (now()),
...
CREATE TABLE `modules` (
  ...
  `installdate` datetime NOT NULL DEFAULT (now()),
  `installedby` varchar(50) NOT NULL,
  `filemoddate` datetime NOT NULL DEFAULT (now()),
...
CREATE TABLE `paylog` (
  ...
  `processdate` datetime NOT NULL DEFAULT (now()),
  PRIMARY KEY (`payid`),
  KEY `txnid` (`txnid`)
);
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `petitions` (
  ...
  `body` text,
  `pageinfo` text,
  `closedate` datetime NOT NULL DEFAULT (now()),
