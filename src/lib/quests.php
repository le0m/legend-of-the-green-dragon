...
function receive_reward($q) {
	...
	if ($reward_exp > 0) output("`^Du erh�ltst `%%s`^ Erfahrungspunkte.`n", $reward_exp);
	if ($reward_gold > 0) output("`^Du erh�ltst `%%s`^ Goldst�cke.`n", $reward_gold);
	if ($reward_gems > 0) output("`^Du erh�ltst `%%s`^ Edelsteine.`n", $reward_gems);
	if ($reward_quest > 0) output("`^Du erh�ltst `%%s`^ Questpunkte.`n", $reward_quest);
